...
public static class MeshHelper {
    ...
    static float minDist = Mathf.Infinity;
    ...
    public static void Subdivide4ByDist(Mesh mesh, float dist) {
        newVectices = new Dictionary<uint, int>();
        ...
        for (int i = 0; i < triangles.Length; i += 3) {
            ...
            float d1 = Vector3.Distance(vertices[i1], vertices[i2]);
            float d2 = Vector3.Distance(vertices[i2], vertices[i3]);
            float d3 = Vector3.Distance(vertices[i3], vertices[i1]);

            float maxD = Mathf.Max(d1, d2, d3);
            float minD = Mathf.Min(d1, d2, d3);
            maxDist = Mathf.Max(maxD, maxDist);
            minDist = Mathf.Min(minD, minDist);
            averageDist += ((d1 + d2 + d3) / 3) / triangles.Length;

            if (Vector3.Distance(vertices[i1], vertices[i2]) > dist || Vector3.Distance(vertices[i2], vertices[i3]) > dist || Vector3.Distance(vertices[i3], vertices[i1]) > dist) {
                int a = GetNewVertex4(i1, i2);
                int b = GetNewVertex4(i2, i3);
                int c = GetNewVertex4(i3, i1);
                indices.Add(i1); indices.Add(a); indices.Add(c);
    ...
    public static void Subdivide9ByDist(Mesh mesh, float dist) {
        newVectices = new Dictionary<uint, int>();
